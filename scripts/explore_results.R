...
# Colorblind-safe palette: Okabe-Ito (first 6) + Paul Tol muted (remaining 6)
ancestry_colours <- c(
  hapnest_AMR     = "#E69F00",  # orange
  hapnest_AFR     = "#56B4E9",  # sky blue
  hapnest_EUR     = "#009E73",  # bluish green
  hapnest_EAS     = "#0072B2",  # blue
  hapnest_CSA     = "#D55E00",  # vermillion
  hapnest_MID     = "#CC79A7",  # reddish purple
  AMR_1kg         = "#DDCC77",  # sand
  AFR_1kg         = "#88CCEE",  # cyan
  EUR_1kg         = "#44AA99",  # teal
  matched_admixed = "#117733",  # green
  gwas_subset     = "#332288",  # indigo
  oracle          = "#882255"   # wine
)
...
# hapnest_AMR, quantitative trait, gaussian effect distribution
...
p7 <- ggplot(var_d, aes(panel_n, mean_val,
  ...
  scale_x_log10(breaks = c(100, 500, 1000, 5000, 10000),
                labels  = comma) +
  labs(title    = paste("PGS posterior variance vs panel size — PRScs",
                        "(hapnest_AMR, quantitative, gaussian)",
                        sep = "\n"),
       x        = "Panel N (log scale)",
       y        = "Mean PGS variance",
       colour   = "h\u00b2",
...
# ── 8. Accuracy vs panel size: hapnest_AMR only ───────────────────────────────
...
p8_base <- function(metric_sel, trait_sel, ylabel) {
  ggplot(
    amr_vals |> filter(metric == metric_sel, trait == trait_sel),
    ...
    scale_x_log10(breaks = c(100, 500, 1000, 5000, 10000),
                  labels  = comma) +
    scale_colour_manual(
      values = c(ldpred2 = "#1F78B4", prscs = "#33A02C")
    ) +
...
save_plot(p8b, "08b_auc_amr_prev10", w = 12, h = 10)
save_plot(p8c, "08c_auc_amr_prev30", w = 12, h = 10)
...
# ── 10. R² vs panel size — by ancestry, one figure per p_causal ──────────────

r2_anc <- d |>
  filter(metric == "R2_full", trait == "quantitative") |>
  mutate(p_causal_f = factor(p_causal, levels = sort(unique(p_causal)))) |>
  group_by(method, panel_ancestry, panel_n, h2_f, effect_dist, p_causal_f) |>
  summarise(mean_val = mean(value, na.rm = TRUE),
            se_val   = sd(value, na.rm = TRUE) / sqrt(n()),
            .groups  = "drop")

for (pc in levels(r2_anc$p_causal_f)) {
  p10 <- ggplot(
    r2_anc |> filter(p_causal_f == pc),
    aes(panel_n, mean_val,
        colour   = panel_ancestry,
        linetype = method,
        shape    = method)
    ...
    scale_colour_manual(values = ancestry_colours) +
    labs(
      title    = "PGS R\u00b2 vs LD-panel sample size by ancestry",
      subtitle = sprintf(
        "p_causal = %s | rows = h\u00b2 | cols = effect dist | avg over reps",
        pc
      ),
      x        = "Panel N (log scale)",
      y        = "Mean R\u00b2 (\u00b1 1 SE)",
      colour   = "LD panel ancestry",
      linetype = "Method",
      shape    = "Method"
    ) +
    theme(legend.position = "right",
  ...
  save_plot(p10,
            sprintf("10_r2_vs_panelN_by_ancestry_pc%s", pc),
            w = 14, h = 10)
}

#!/usr/bin/env Rscript
# Plot binary-trait AUC_incremental vs panel sample size,
# one line per h2, for the AMR panel only.
# Separate PNGs per (trait, method, effect_dist); facet by p_causal.
#
# Usage:
#   Rscript scripts/plot_binary_auc_by_h2.R \
...
acc <- d |>
  filter(startsWith(trait, "binary"),
         metric == "AUC_incremental",
...
agg <- acc |>
  group_by(trait, method, effect_dist, p_causal, h2, panel_n) |>
...
combos <- agg |>
  distinct(trait, method, effect_dist) |>
  arrange(trait, method, effect_dist)

for (i in seq_len(nrow(combos))) {
  tr <- combos$trait[i]
  m  <- combos$method[i]
  ed <- combos$effect_dist[i]
  sub <- agg |> filter(trait == tr, method == m, effect_dist == ed)
  ...
  p <- ggplot(sub, aes(panel_n, mean_val,
    ...
    labs(x = "LD-panel sample size",
         ...
         title = sprintf("%s AUC vs panel N  |  %s  |  %s  |  AMR panel",
                         tr, m, ed))

  out <- file.path(outdir,
                   sprintf("12_%s_auc_vs_panelN_by_h2_AMR_%s_%s.png",
                           tr, m, ed))

...
## Result 2 — Comparing incremental $h^2$ across heritability values

![](../results/summary/plots/11_quant_r2_vs_panelN_by_h2_AMR_ldpred2_spikeslab.png){fig-align="center" width="72%"}

At **low $h^{2}$** all curves collapse — the trait is hard regardless of panel. As $h^{2}$ grows, the gap between panels widens.
...
::: {.incremental}
- **Sparse architectures** ($p_{\text{causal}} \le 0.01$).
- **High heritability** ($h^{2} \gtrsim 0.4$).
...
- **Low heritability** ($h^{2} \le 0.2$) — accuracy is floor-limited; panel choice is a second-order issue.
- **Very large panels** ($N = 50{,}000$) — ancestry differences are largely absorbed.
:::
...
- We ignored **selection** and pleiotropy.
